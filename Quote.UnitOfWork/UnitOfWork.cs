...
namespace Arch.EntityFrameworkCore.UnitOfWork
{
    ...
    public class UnitOfWork<TContext> : IRepositoryFactory, IUnitOfWork<TContext>, IUnitOfWork where TContext : DbContext
    {
        ...
        public Task<IEnumerable<T>> FromSqlDapperAsync<T>(string sql)
        {
            return _context.Database.GetDbConnection().QueryAsync<T>(sql);
        }

        public Task<List<TEntity>> FromSqlAsNoTracking<TEntity>(string sql) where TEntity : class
        {
            var ls = _context.Set<TEntity>().FromSqlRaw(sql).AsNoTracking().ToListAsync();
            return ls;
        }

        public IQueryable<TEntity> FromSql<TEntity>(string sql) where TEntity : class => _context.Set<TEntity>().FromSqlRaw(sql);
